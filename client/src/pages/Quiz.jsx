...
function Quiz() {
  ...
  useEffect(() => {
    if (!nickname || !initialQuestion || !initialPlayers) {
      // Redirect to home if state is missing (e.g., direct access)
      navigate('/');
      return;
    }

    // Initialize scores from initialPlayers
    const initialScores = {};
    ...
    socket.on('next-question', (data) => {
      ...
      setSelectedAnswerIndex(null); // Reset selected answer for new question
      setFeedback('');
    });
    ...
    socket.on('player-left', ({ playerId, nickname: leftNickname, players: updatedPlayers }) => {
      ...
      // Optionally handle game end if only one player remains
    });
